...
class EnemyShip(Enemies):
    ...
    def __init__(self):
        ...
        self.image = image.load("./assets/yellow_placeholder.png")
    ...
    def displacement(self):
        ...
        if self.rect.y >= 450 or self.rect.y <= 10:
            ...
            if self.rect.x > 100:
                # This condition verifies if the ship isn't too much to the left. If its x coordinate is too low,
                # it will move it leftward anymore, or move it to the strict minimum x coordinate. This is to let
                # a space for the player to move.
...
class Sinusoid(Enemies):
    ...
    def displacement(self):
        ...
        if not self.trajectory:  # This boolean expression was proposed by the IDE to detect when this list is empty.
            ...
            if self.rect.x > 100:
                # This condition verifies if the ship isn't on the left. If its x coordinate is below designated level,
                # it will not lower again its altitude, or place it at the minimal x level.
                if self.rect.x - 8 * self.velocity < 100:
                    self.rect.x = 100
                else:
                    self.rect.x -= 8 * self.velocity  # Sqrt attenuation isn't used here due to the general low velocity
                    # of the sinusoid
            if (self.rect.y - 450) > -400:
            ...
            if self.reached_border:
                ...
                # Same as above, except control points are located relative to the upper border (y = 0).
                for i in range(0, 10):
                    self.trajectory += bezier_curve_calc(
